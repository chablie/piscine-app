...
// Sélectionne plusieurs lignes selon une égalité simple (colonne = valeur)
export async function selectPlusieurs(table, colonne, valeur, select = '*') {
  const rep = await fetch(
    `${SUPABASE_URL}/rest/v1/${table}?${colonne}=eq.${encodeURIComponent(valeur)}&select=${encodeURIComponent(select)}`,
    { headers: headers() }
  );
  if (!rep.ok) throw new Error(`selectPlusieurs ${table} : ${rep.status}`);
